...
## DICOMDIR Generation (CRITICAL for medical workstations)
...
### How it works:
- `setup.ps1` downloads dcmtk 3.7.0 (~9 MB) from dicom.offis.de into `tools/dcmtk/`
- `burn.ps1` runs `dcmmkdir.exe` from disc staging root after all files are in place
- DICOMDIR is placed at disc root (`D:\DICOMDIR`), referencing paths like `WEASIS\DICOM\DIR000\filename`

### dcmmkdir command:
```
cd /d <staging-root>
dcmmkdir.exe +r +id Weasis\DICOM +D DICOMDIR +I -Pgp
```
- `+r` = recurse subdirectories
- `+id Weasis\DICOM` = input directory (relative to CWD = disc root)
- `+D DICOMDIR` = output file at CWD (disc root)
- `+I` = invent missing Type 1 attributes (tolerant mode for real-world DICOM)
- `-Pgp` = General Purpose profile (CD-R/DVD-R/BD)

### Critical notes:
- **Must run from disc root** — DICOMDIR stores paths relative to itself
- **PACS ZIP DICOMDIR is excluded** — has paths for `viewer-mac.app/Contents/DICOM/...`
- **Fallback**: if dcmmkdir not available, disc still works with Weasis (LoadLocalDicom)
- **DCMDICTPATH** env var set to `tools/dcmtk/share/dcmtk/dicom.dic` before running
- **Paths in DICOMDIR**: `WEASIS\DICOM\DIR000\filename` (relative to disc root)
...
├── DICOMDIR              ← GENERATED by dcmmkdir (index for medical workstations)
...
└── Weasis/
    ...
    └── weasis-viewer.bat
```
DICOMDIR at disc root references `WEASIS\DICOM\DIR000\filename` — recognized by Siemens, GE, Philips workstations.
